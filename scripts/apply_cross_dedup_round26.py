# -*- coding: utf-8 -*-
"""apply_cross_dedup_round26.py — 完全差异化改写最后 12 个重叠。"""
...
REWRITES = {
    # ESTP-decision: 共享 "成年人选 A 是为了在 A 上拿到足够" + "Ti 的判断力落在选一条吃透"
    ("ESTP", "ESTP-faq-decision"):
        "ESTP 在重大抉择中总想再看一眼其他选项——Se 不肯放过任何可上手的方案，Ti 又想给每个方案算期望值。承认一种现实：成年人选 A 是为了在 A 上拿到足够硬的东西，B 没那么可惜。深度比广度更划算——选一条吃透是 ESTP 难得的训练。",
    # ESTP-social-drain: 共享 "重复寒暄、低信息量聚会也耗电" + "选高质量社交，而非来者不拒"
    ("ESTP", "ESTP-faq-social-drain"):
        "ESTP 社交本来充电不耗电——但条件得是有新冲击。重复寒暄、低密度聚会也耗电：Se 缺乏新素材可接、Ti 没有新观点可辩就空转。优先挑能 push 你思路的人，把每周社交时间写进日历主动管理；社交才不会从能量源变成隐形漏电。",
    # ISFP-breakup: 共享 "能性。把前任从神坛上"
    ("ISFP", "ISFP-faq-breakup"):
        "ISFP 分手后容易把前任符号化为唯一的灵魂伴侣——Fi 的高强度铭刻把过去细节全部存档，Se 又在每次路过某些地方时把它们调出来。承认心动真实，但神化会挡光。主动把前任从记忆的神坛上撤下来——铭刻越深越要主动把光从过去切回现在。",
    # INFP-weakness-01 vs ESFP-weakness-03: 共享 "被强烈情绪裹挟时会做出事后后悔的决定——要识别『我现在在情绪里』的信号"
    ("INFP", "INFP-weakness-01"):
        "被强烈情绪卷入时容易做出事后后悔的决定——识别『我现在情绪上头』的标志是 INFP 的基本功，重大决策不在这时候做。情绪里的决定是替情绪买单，不替未来的你。",
    # ISFP-faq-conflict: 共享 "冲突当下会沉默——Fi 的保护" + "核心想说的写下带身上"
    ("ISFP", "ISFP-faq-conflict"):
        "ISFP 在冲突当下会沉默——Fi 的本能反应是把感受封起来慢慢消化，Se 又把对方的表情逐字存档等合适的时机再说。过几天才反刍出想说的话，常常已经错过了最好的时点。重要争执前把核心想说的写下带身上——落笔也算数，把感受变成外部证据。",
    # ISFP-faq-criticism: 共享 "批评后容易把一句否定扩大为" + "区分『行为与身份』"
...
def main():
    ...
    scan_path = TMP_DIR / "m4-cross-r26.json"
    env = os.environ.copy()
    ...
    if data["cross_personality"]:
        print("\n=== remaining ===")
        for g in data["cross_personality"]:
            print(f'  {g["fragment"]!r:40} {[(e["personality"], e["id"]) for e in g["entries"]]}')
