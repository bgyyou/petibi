# -*- coding: utf-8 -*-
"""apply_cross_dedup_round22.py — 收尾：处理 round 21 跳过的最后 13 个。"""
...
REWRITES = {
    # ENTP-ENFP strength-01
    ("ENTP", "ENTP-strength-01"):
        "ENTP 在需要打破常规的场合（产品盲点辩论、创意突围），能在半小时内抛出多个反向角度——Ne 抓可能性、Ti 做底层检验，反向思考是 ENTP 的招牌。",
    # ESTP-decision
    ("ESTP", "ESTP-faq-decision"):
        "ESTP 在重大事项前总觉得选 A 就堵死了 B 的路——Se 总在抓所有可见的选项，Ti 又想去算每个选项的可能性。承认一个事实：成年人选 A 是为了在 A 上拿到足够硬的东西，B 没那么可惜。把决策看成收口，深度比广度更划算。",
    # ESTP-social-drain
    ("ESTP", "ESTP-faq-social-drain"):
        "ESTP 社交本来充电不耗电——但条件得是有新冲击。重复寒暄、低信息量聚会也耗电：Se 缺乏新素材可接、Ti 没有新观点可辩就空转。选高质量社交而非逢邀必到——脑力型社交是 Se 主导者的最低门槛，把每周社交时间写进日历主动管理。",
    # ISFP-breakup (116 → 120+)
    ("ISFP", "ISFP-faq-breakup"):
        "ISFP 分手后容易把前任符号化为唯一的灵魂伴侣——Fi 的高强度铭刻把过去的细节全部存档，Se 又在每次路过某些地方时把它们调出来。承认心动真实，但神化会挡住后面所有可能性。把前任从神坛上撤下来——铭刻越深越要主动把光从过去切回现在。",
    # INFP-weakness-01
    ("INFP", "INFP-weakness-01"):
        "被强烈情绪卷入时会做出事后后悔的决定——要识别『我现在情绪上头』的信号，重大决策不在这时候做。情绪里的决定是替情绪买单，不替未来的你。",
    ...
    ("ISFP", "ISFP-faq-criticism"):
        "ISFP 听到批评后容易把一句否定扩大为整体否定——Fi 视角的整体判定模式，Se 又把当时的表情语气逐字存档当证据。区分『行为与身份』：分歧指向行为，并非指向你这个人。把感受从事后自证切到事前可改——身份不动，行为能改。",
    # ESFP-cognitive-02
    ...
    ("ESFP", "ESFP-faq-public-speaking"):
        "ESFP 在讲台前一概不怯场——气氛越热烈越兴奋，Se 在现场能拿满档刺激。但想法比嘴快半拍——现场气氛一上去，Se 会跑去接观众的笑声而忘了主线。提前把三个要点写在台口余光能扫到的地方，请朋友在 25 分钟、50 分钟举牌——把 Se 关进结构里的硬工具，别让好看盖过讲清。",
    # ISTJ-relationship-01
...
def main():
    ...
    scan_path = TMP_DIR / "m4-cross-r22.json"
    env = os.environ.copy()
