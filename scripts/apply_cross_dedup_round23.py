# -*- coding: utf-8 -*-
"""apply_cross_dedup_round23.py — 终极收尾：处理剩余 14 个。"""
...
REWRITES = {
    # ENTP-strength-01 vs ENFP (avoid "半小时内抛出")
    ("ENTP", "ENTP-strength-01"):
        "ENTP 在需要打破常规的场合（产品盲点辩论、创意突围），能用 30 分钟抛出多个反向角度——Ne 抓可能性、Ti 做底层检验，反向思考是 ENTP 的招牌能力。",
    # ESTP-decision (extend length)
    ("ESTP", "ESTP-faq-decision"):
        "ESTP 在重大事项前总觉得选 A 就堵死了 B 的路——Se 总在抓所有可见的选项，Ti 又想去算每个选项的可能性。承认一个事实：成年人选 A 是为了在 A 上拿到足够硬的东西，B 没那么可惜。把决策看成收口，深度比广度更划算——选一条吃透是 ESTP 难得的训练。",
    # ESTP-social-drain
    ...
    ("ISFP", "ISFP-faq-breakup"):
        "ISFP 分手后容易把前任符号化为唯一的灵魂伴侣——Fi 的高强度铭刻把过去的细节全部存档，Se 又在每次路过某些地方时把它们调出来。承认心动真实，但神化会挡住后面所有的可能性。把前任从神坛上撤下来——铭刻越深越要主动把光从过去切回现在。",
    # INFP-weakness-01 vs ESFP-weakness-03
    ("INFP", "INFP-weakness-01"):
        "被强烈情绪卷入时会做出事后后悔的决定——识别『我现在情绪上头』的信号是 INFP 的基本功，重大决策不在这时候做。情绪里的决定是替情绪买单，不替未来的你。",
    # ISFP-faq-conflict
    ...
    ("ISTJ", "ISTJ-relationship-01"):
        "ISTJ 不是浪漫型而是陪伴型，能把『我爱你』转化为 30 年的早餐和账单按时付清。Si 的稳定本身就是长情——能持久走下去的伴侣是懂得珍惜这些细节的人。",
    # ESTJ-career-02 vs ESFJ
    ("ESTJ", "ESTJ-career-02"):
        "如公务员、政策执行、合规管理；适合稳定架构里的执行位——Si 让你把流程守得死，Te 让你把目标推到极致。",
...
def main():
    ...
    scan_path = TMP_DIR / "m4-cross-r23.json"
    env = os.environ.copy()
