...
export interface AppDeps {
  ...
  /** M4 内嵌场景：外部指定 public/ 目录（resources/server/public），优先于 import.meta.url 推算 */
  publicDirOverride?: string
  /** M4 内嵌场景：外部指定海报存放目录（userData/posters），优先于 import.meta.url 推算 */
  postersDirOverride?: string
  /** 内嵌场景：外部指定人格速查卡目录（resources/data/personas），优先于 env / import.meta.url 推算。
   *  不注入时打包版会读不到速查卡，「我的」页动物名会退化成"未知/伙伴"兜底文案。 */
  personasDirOverride?: string
}
...
export function createApp(deps: AppDeps): Express {
  ...
  // 路径解析优先级：publicDirOverride（内嵌场景）> import.meta.url 推算（dev/CLI 场景）。
  // 内嵌场景下 server bundle 已被 esbuild 打成单文件，import.meta.url 指向 bundle 自身，
  // 无法再用相对路径解析到 public/；必须由主进程在 startServer 时注入绝对路径。
  const publicDir =
    deps.publicDirOverride ??
    join(dirname(fileURLToPath(import.meta.url)), "..", "public")
  const postersDir =
    deps.postersDirOverride ??
    join(dirname(fileURLToPath(import.meta.url)), "..", "data", "posters")
  ...
  // 内嵌场景下 postersDir 落到 userData；CLI 场景下走 import.meta.url 推算到 server/data/posters。
  app.use("/data/posters", express.static(postersDir))
  ...
  app.use("/api/me", auth, createMeRouter({ db, personasDir: deps.personasDirOverride }))
  ...
  app.use(
    "/api/posters",
    createPostersRouter({ db, config, moderation, auth, postersDir: postersDir }),
  )
